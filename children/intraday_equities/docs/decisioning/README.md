...
| A18868 | execute | Gate 6: versioned event and metric contract (ADR-0117, proposed) | 2026-09-10T12:22:32+00:00 | docs/plans/2026-09-08-final-model-replay-and-monitoring.md §6 Phase 6; docs/architecture/decision-log.md ADR-0114 Phase 6 | docs/architecture/decision-log.md ADR-0117; dskit/production/{vocab,metrics,monitors}.py; intraday_equities/metrics.py; tests/test_metrics.py |  | Additive only. vocab gains EVENT_SCHEMA_VERSION/EVENT_CATEGORIES/EVENT_FIELDS (plan §6 catalogue verbatim), UNBOUNDED_LABEL_FIELDS, EVENT_READINGS and 13 safe-aggregate metric names; monitor_verdicts_total widened to all 19 monitor kinds. metrics.py gains EventCatalogue, EventAdapter, EventReadings, the polymorphic _Metric.record hook, and a refusal of symbol/lead as a label name. monitors.py gains Completeness. Child gains EquityEventAdapter + SignalDecay. Values are recorded only: no WARN/HOLD threshold and no alert rule (plan §11 items 7 and 10 stay open). No market data read, no run executed. |
