...
- All data is explicitly synthetic. No provider pulls, training, HPO, replay,
  ...
  ADR-0187 supersedes, for the ETF grid only, "Only European-exercise, PM
  cash-settled ... index condors" (SPY/QQQ/IWM options are American and
  physically settled: CondorQuoteBacktest values them at the European payoff
  minus a conservative early-exercise charge) and "Do not add ... readers" for
  ChainQuoteRows alone. The rules contracts.py owns are module functions
  (quote_problems, condor_credit, american_short_charge) that the synthetic
  cashflow owner and the quote backtest both call; the grid documents are
  GENERATED by index_options.grid and pinned to it by test — never hand-edit
  configs/grid/.
