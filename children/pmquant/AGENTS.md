...
# AGENTS.md — pmquant (a dskit child)

Agent orientation — see README.md for what the child does and how to run it.
...
- **Import = registration**: `pmquant/__init__.py` imports `nodes_data`,
  `nodes_model` and `nodes_capital`, which register the kinds. `--adapter pmquant` is exactly this
...
## Gotchas that cost a run tonight

- **Declare the optimizer.** The torch pack defaults to plain SGD, which
  never left the uniform prediction on this recipe (log-loss 0.636 =
  ln 3 / 3 at 3, 15 and 40 epochs). Both shipped documents name
  `torch.optim.AdamW`; a new training document must too.
- **Fee rates are per SERIES and fail-closed.** `fee_rate_by_series`
  needs an entry for every series the sizer may touch (a scalar, or
  dated cases keyed on the market's close instant); there is no wildcard
  and no default — a missing rate refuses by name at run time.
- **The gate halts the sizer.** A NO-GO from `stat_test` halts `size`
  and `run_report` (exit 3). That is the design; the e2e test asserts GO
  because the synthetic world is BUILT mispriced.
- **Spent record streams are released** (ADR-0048): `pred_rows` and the
  like read back from `result.outputs` as summaries; assert on metrics
  and evidence.
- **Never pool cells.** Every (series, lead, rung) is its own hypothesis;
  the stat test corrects across series, it never averages them.
- **`stream()` is the hook**, not `params["stream"]`: the readers pin
  their stream default by overriding `ObservationRows.stream()`.
- **Two LeadGrids would be one too many.** `pmquant.ladder.protocols.LeadGrid`
...
```
pmquant/               # tier-3 code — see README.md for the file-by-file tree
├── nodes_data.py      #   data/labels/report kinds over the observations seam
├── nodes_model.py     #   panels / train / predict / ensemble / signal kinds
├── nodes_capital.py   #   pmquant-kelly-mio over the PyomoSolve doorway
├── books.py fees.py mio.py models.py testing.py
└── ladder/            #   protocols.py (vocabularies) + panels.py (features)
configs/               # run-e2e + run-kalshi-ladders (twins); 5 sources; suite; asset-model
