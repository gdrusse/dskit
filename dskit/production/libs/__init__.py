...
One module per library: ``parquet.py`` and ``sqlite.py``, and phase 3's
``exchange_calendars.py``, ``prometheus.py`` and ``opentelemetry.py``.
...
``monitors.REFERENCE_KINDS``, ``sqlite.py`` registers ``sqlite`` into
``ledger.LEDGER_KINDS``, ``exchange_calendars.py`` registers ``exchange``
into ``sessions.CALENDAR_KINDS``, and the two metric exporters register
into ``metrics.METRIC_SINK_KINDS`` — which is why the phase-3 packs add no
registry at all (§4.3). Import is registration, so a document that names
