...
class RunAttestation:
    ...
    a run directory's outputs as evidence (ADR-0119, extending ADR-0116):
    ...
    **Never compose `completed()`, `node_completed()` and
    ...
    (ADR-0119 follow-up), against the identity being claimed. It cannot
    defend against an attacker who edits `nodes/*.json` itself — no
    hash-chain ties a node record to its neighbors or to
    ``resolved.json`` (a real gap ADR-0119 disclosed and left open); a
...
def _write_node_records(run_dir, the_plan, run, document_hash):
    """Write one JSON record per node, in execution order, each stamped with the run's own ``document_hash`` (ADR-0119 follow-up)."""
