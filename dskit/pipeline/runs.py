...
#: The per-row loss gaps ADR-0063 pools, written by a score node
...
def read_skill_series(run_dir):
    ...
    ADR-0063's evidence: one entry per ``(series, lead)`` carrying ``d``
...
def read_curve_records(run_dir):
    ...
    carried. Enough for the across-fold half of ADR-0063 and for the
...
def _pooled_row(series, lead, folds, records, alpha):
    """One ADR-0063 verdict row from per-row loss gaps."""
...
def score_walk(summary_dir, alpha=0.05, group="GROUP"):
    """Judge a walk-forward under ADR-0063 — per series and as a group.
    ...
    if not exact:
        notes.append(
            ...
            "per-row loss gaps (ADR-0063 artifact absent) — the pooled DM "
