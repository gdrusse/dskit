...
| C2 | MERGED | eccbaa7 | ADR-0040 — the biggest card of the run (5 rounds, 20 agents). Closes 4 TODO items + audit HIGH-2. The child's gap semantics were PINNED FIRST, proven able to fail twice, then the rewrite landed under the SAME pin untouched — the parity proof. Two MAJORs closed at the cap: a label/lag column COLLISION that silently put a forward value in a lag column (invisible to the causality guard, which was told the column was forward) now refuses at plan; and the fitted base no longer promises row order it does not deliver. Review went further than asked — `price_ok`/`lead_frac_ok` were the 3rd and 4th copies of one numeric rule, now narrowings of `records.number_ok`. THREE ignore entries drained (numpy.py, child nodes.py, records.py). Hash gate empty. Rebase across C4/C3/C7 took 3 additive conflicts + one D401 caused by C3 draining planner.py |
...
| C5 | MERGED | 75e38cf | ADR-0041. `torch_ts.py` catalog; `torch.py` byte-identical (`0c908fa8…`). 10 archs, `arch` swept. Child names `lstm`, NextBarLSTM deleted, live restores via `TimeSeriesTrain.build_module`. Child hashes moved as declared. pipeline+libs 2648/109. |
| C6 | MERGED | 2be1918 | ADR-0042. FeatureSelector + sklearn-select (doorway) + torch-importance (input-gradient). torch.py DRAINED (91 D → 0). Flows 1 and 3 e2e. Hash gate empty (17 docs). Suite 2637/109 pipeline+libs (main was 2554/109; skip count unchanged). Flow 2 unblocked by ADR-0044. |
| ADR-0044 | ACCEPTED | c009953 | Per-knob fitted_transform searchability. `unsearchable_space_why` is the one check (planner + driver). Member knobs plan and execute; `FittedTransform._PARAMS` still refuse. |
...
| D1 | MERGED | e49dfcb | selection-demo.json: VarianceThreshold on mid+noise, sweep LinearRegression vs Ridge, winner consumes `$select.features`. Flow 2 (space over selector knob + estimator) plans and runs via ADR-0044. Ledger 18. |
...
| D4 | DONE | ae9d3a5 | TODO marked; baseline block refreshed; integrity sweep clean (52 checked, 24 open, zero landed-notes-under-unchecked-boxes) |
| D5 | DONE | (this commit) | RE-ENTRY rewritten with PICK UP HERE at the top: the three remaining cards, their accepted ADR specs, the worktree+venv recipe, the merge law, the rulings not in the cards, and the verification recipe. Self-sufficient in-repo — it does not depend on any file outside the repository |
