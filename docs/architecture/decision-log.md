...
## ADR-0038 — `mlflow`: the tracking-sink pack (tier 2)

**Status:** accepted (2026-08-28) — closes the TODO "Running
experiments" item *no tracking sink ships*. Written after the code in
this branch, which is the wrong order; the entry stands as the record,
and the rulings below are the ones a reviewer must be able to
challenge.

**Context.** The engine has always had the whole `Tracker` seam and
none of the destination: `protocols.Tracker` says what a sink must do,
`register_sink_kind`/`SINK_KINDS` says how one is claimed, a document's
`tracking.sinks` says which ones a run opens — and the only sink that
ever registered was the in-memory one `testing.register_synthetic()`
puts up for tests. Every run's params and metrics died with its run
directory; cross-run comparison did not exist.

The gotcha that shapes the whole design: `driver._Trackers` SWALLOWS
every per-sink exception, deliberately — telemetry must never kill a
run the driver promised to record. The cost is that a misconfigured
sink logs nothing and SAYS nothing: the run reports success and the
experiment is simply absent.

**Decision.** A tier-2 pack `libs/mlflow.py` — `MlflowTracker`, kind
`"mlflow"`, claimed by an explicit idempotent `register()` exactly as
the `memory` sink is (real sinks register application-side). Four
rulings that are not mechanical library wrapping:

1. **The pack fills `SINK_KINDS`; `NODE_KINDS` stays empty.** A
   tracking destination is not a step of the pipeline, and an empty node
   table keeps the pack's vocabulary out of `pipeline`, the section
   identity is computed over. The pre-existing `tracking` SECTION *is*
   hash-graded (`DOC_NON_IDENTITY_SECTIONS` is `env`/`outputs`/
   `schedule` only). Arguably it should read like `outputs` and be
   excluded — but that is core's change, not a pack's, and it is not
   cheap: `to_obj` emits `"tracking": null` always and `config_hash`
   pops excluded keys, so the move rewrites EVERY document's hash
   (measured on `examples/pipeline/mpl-figure.json`, which declares no
   tracking at all: `e9d5f60c…` -> `314cea4d…`), orphaning every run
   directory and stored artifact. Deferred to its own ADR plus a
   baseline re-cut, and pinned meanwhile by `TestHashPlacement`.
2. **Everything checkable is checked where the swallow cannot reach** —
   default-deny params AND a stdlib reachability probe of the tracking
   URI inside `validate_params`, which runs at document PARSE
   (`SinkConfig.__post_init__`). Network I/O during parse is unusual and
   deliberate: an unreachable destination refused at plan time is a
   sentence a human reads; the same destination failing at run time is
   silence.
3. **A CLOSED scheme vocabulary** — `""`, `file`, `http`, `https`,
   `sqlite`. mlflow also speaks postgres and mysql; this pack refuses
   them rather than approve a destination it cannot prove reachable,
   because an unprovable destination is one whose misconfiguration is
   silent. `probe_destination` is the subclass hook for anyone who
   wants another store family.
4. **Misconfiguration and a DEGRADED server are different failures.**
   `_open_sinks` runs OUTSIDE `run_document`'s try, so a sink raising at
   construction aborts the run before any node executes — the one thing
   the swallow exists to prevent. So mlflow missing, a non-active
   experiment, or any failure of a LOCAL store (whose writability the
   probe already proved) raises `ConfigError`, while an `http`/`https`
   store that passed the TCP probe and then fails DISABLES the sink and
   logs a warning. `connect_timeout` bounds mlflow's own HTTP calls as
   well as the probe (its request-timeout and retry environment
   variables, scoped to this sink's calls and never over a value the
   operator set) — without that, a reachable but 5xx-ing server stalls
   `run_document` for minutes ahead of every node.

Supporting choices. The mlflow RUN is created LAZILY on the first log:
the driver builds sinks before resolve finishes and closes them on
every pre-execution refusal (an occupied run directory is the
documented normal case), so an eager run left empty `FINISHED` runs
that browse exactly like successful ones. The default store is
`sqlite:///mlruns.db`, not the older `./mlruns` directory, which
current mlflow refuses unless `MLFLOW_ALLOW_FILE_STORE` is set; the
directory spellings stay in the vocabulary for mlflow 2.x, and the pack
never opts anyone in. Param and metric NAMES are capped like values
(the store caps both, and `log_batch` is all-or-nothing, so one
over-long flattened path would cost every param in the call), a capped
name carrying the full name's digest so two deep paths cannot collide.
Metrics carry a per-key increasing step, because mlflow breaks a
latest-value tie by max(step), then max(timestamp), then max(VALUE) —
at a fixed step a restated metric reports the LARGER value, and the
driver restates by design (spec §8).

**Consequences.** A document gains a `tracking` section and its runs
land in mlflow with E1's flattened `<node>.<param.path>` params, so a
sweep is filterable by hyperparameter. Nothing changes for a document
that declares no sink, and no identity hash moves. Two limits are
recorded rather than fixed: `Tracker.close()` carries no status, so a
crashed run still reads `FINISHED` (giving it one is a core change to
`protocols.py`, both driver call sites and the `memory` sink); and a
disabled sink is a warning in the log, not a run-visible result.
