...
2. A trusted roster publisher derives exact canonical
   ...
   no fresh F4 write is authorized. No external source provider or raw source read occurs in this roster stage. Reading back only the newly written roster WORM member through the fixed F4 provider for publication verification is allowed and cannot reach raw source bytes. The shared reserve and dynamic trusted root
...
## ADR-0138 - fixed synthetic roster publication and v2 receipt profile

**Status:** accepted (2026-09-16; owner standing approval and autonomous-completion direction); fresh independent Phase 0 cleared with zero Critical/Major/Minor before effecting RED. This instantiates ADR-0135/0136 for nondeployment synthetic bytes only. It grants no raw source access, P4 admission, Packet8 admission or deployment.

**Context.** ADR-0135 defines the bootstrap-to-roster chronology but does not pin the nondeployment producer identity, receipt signer, or exact effecting adapter. The existing development F4 broker is process-local and appends its PUBLISHED lifecycle record before writing WORM member bytes. A partial write must never be treated as publication. ADR-0136's first SQLite slice reserves a signed ID but exposes no F4 transition permit. The P4 fixed TEST signer/time/revocation corpus cannot issue or verify a live roster/v2 receipt.

**Decision.**

1. One private trusted roster publisher owns the fixed ADR-0136 reserve, one F4 broker instance, and a distinct outer RootPublicationReceipt.v2 put-if-absent store. Caller input is only original canonical bootstrap authorization and G1/G2 grant bytes. The caller cannot select the database, F4 provider, signer, clock, revocation source, producer identity, root, output path, receipt key, policy, or intent. The publisher derives canonical SourceRosterCapture.v1 bytes from the sorted authorized source IDs and scope; the only output member is `source_roster.json` with media type `application/json`. Purpose is `source-roster`, producer node is `source-roster-capture`, output is `source_roster`, snapshot_version is `v1`. Let A be lowercase SHA-256 of the exact canonical bootstrap authorization bytes. Derive root_ref as ASCII `synthetic-roster/` + A; root_id as lowercase SHA-256 of ASCII `dskit.synthetic-roster-root-id/v1:` + A; producer_run_identity as ASCII `synthetic-roster-run/` + A; and producer_document_sha256 as SHA-256 of exact canonical ASCII JSON `{"schema":"dskit.synthetic-roster-producer/v1","bootstrap_sha256":A,"producer_node":"source-roster-capture","producer_output":"source_roster","purpose":"source-roster"}`. The F4 producer session must carry that same derived run identity, and F4 produce must retain and compare it. No caller-supplied F4 coordinates are accepted. The publish intent is exact canonical ASCII JSON of the closed object `{schema_version:"dskit.synthetic-roster-publish-intent/v1",bootstrap_id,bootstrap_sha256,g1_grant_sha256,g2_grant_sha256,roster_sha256,roster_byte_length,source_rank_policy_sha256,expected_members:[{relative_path,media_type,sha256,byte_length}],root:{root_ref,root_id,snapshot_version},producer:{run_identity,document_sha256,node,output,purpose},output_member,receipt_key:{receipt_schema,publication_authorization_ref,producer_run_identity,producer_document_sha256,producer_node,producer_output,root_ref,root_id,snapshot_version}}`, with no extra keys, exact types and the one fixed output member. The receipt_key ref is the exact tagged JSON object; its ADR-0125 byte component is its canonical ASCII serialization, never a caller byte field. The publisher, reserve transition gate and later root proof independently rederive and compare this whole intent from original bootstrap/grants, fixed profile, retained F4 facts and receipt key; accepting an arbitrary 64-hex digest as authority is forbidden. SHA-256 of those canonical intent bytes is the ADR-0136 reservation intent.
2. The publisher calls the private reserve once. Under BEGIN IMMEDIATE, each of SESSION_STARTED, PRODUCED, SEALED, PUBLISHED and SESSION_ENDED advances exactly from its predecessor, rechecks original G1/G2 signatures, shared current revocation/time and exact stored ID/digests/intent, appends its audit row, and durably commits before its one immediate in-call F4 operation. It receives no reusable public permit. A failure, timeout or ambiguous COMMIT issues no F4 operation. Any F4 exception, partial member write, audit mismatch or uncertain result immediately terminalizes the local publish capability and attempts a durable QUARANTINED audit; no retry/reopen/second F4 operation follows even if SQLite is unavailable. After restart, every nonterminal reserved row without a proven complete persisted publication refuses and cannot regain a capability. The current process may recognize only its original retained F4 tokens and WORM member bytes. It may read back exactly the newly written roster WORM member through its fixed F4 provider to verify the publication; this is not an external source or raw-dataset read. On process restart, the process-local F4 state cannot prove identity, so the spent ID remains unusable and recovery refuses; a later durable F4 resolver needs its own reviewed ADR. A PUBLISHED lifecycle record alone is insufficient: the publisher byte-compares the one actual WORM output member and complete sealed member manifest before receipt admission.
3. The roster RootPublicationReceipt.v2 retains ADR-0125's exact root-receipt field set, with `schema=dskit.root-publication-receipt/v2`, `kind=dataset-capture`, `capture_kind=source-roster`, and `publication_authorization_ref={kind:roster-bootstrap,roster_bootstrap_authorization_sha256:<digest>}`. It has no study_id field; the later root PIS carries the fixed synthetic study_id. Its issuer_role is `data-publisher`, key_usage is `root-publication-bootstrap-g1-g2`, key_id is `data-publisher/root-publication-bootstrap-g1-g2`, and key_version is integer 1. The fixed nondeployment Ed25519 public key is `03ad7440941bf1c06b1d7c326f4d37d2a3c1abd514a9c1f98aa8ed03858731cd`; its private fixture seed is SHA-256 of ASCII `dskit.synthetic-roster-root-publication/v1` and is unavailable as a runtime injection point. Existing v1 key/use and fixed P4 TEST corpus remain untouched.
4. The roster-root-publication IssuanceBasis.v2 has exactly ADR-0135's three sorted parent refs (one bootstrap authorization and role-matched G1/G2 grants), construction-owned `study_id=synthetic-study`, publish_intent_sha256 and signed suffix. It is constructed only from retained original bytes and the fixed profile. Its issued_at_ms and not_before_ms equal the trusted receipt-admission instant; expires_at_ms is the minimum of the bootstrap/G1/G2 expiry values and must strictly exceed that instant. Its revocation snapshot is the current ADR-0136 shared canonical revoked-set digest. The receipt uses that same window, snapshot, signer key and issuance_basis_sha256. Both basis and receipt self-digests hash their exact canonical ASCII objects omitting only their own self-digest and signature; Ed25519 signs those same bytes. Under one BEGIN IMMEDIATE receipt admission the publisher obtains one trusted admission time, derives and freezes unsigned basis and receipt fields from that time plus retained original bytes and F4 facts, and rechecks original dual grants, basis refs/study ID/window/preimage, shared revoked set including the data-publisher issuer/key, live F4 bytes/manifest/root and exact reserved key/intent. It reads trusted time again immediately before commit to refuse expiry or revocation that is already visible under the writer lock, then commits RECEIPT_ISSUED/audit before any basis or receipt signature. Only after that successful durable commit does the fixed signer sign the one retained basis preimage, verify that signed basis against the retained fields, then sign and put the one receipt immediately in-call. No other effect or time-dependent recomputation intervenes; signing or put failure never permits another attempt. Sign-before-put failure leaves no recoverable receipt and forbids another sign or put. The outer receipt store accepts one byte sequence at the exact ADR-0125 WORM key; a byte-identical put-if-absent returns the original and changed bytes refuse. The effecting publisher itself never invokes put twice. A successful publisher returns only retained identity bytes for later read-only proof, never a P4 or raw permit.
5. The outer receipt store and F4 provider may be process-local for this nondeployment slice because restart recovery is terminal refusal. They must not be confused with the F4 lifecycle receipt log, whose stream key and append-only semantics differ. A later durable resolver can replace this refusal only under a separately reviewed contract. ADR-0137 reads the retained one original receipt and F4 stream; it cannot manufacture missing evidence from caller JSON. Post-roster G1/G2 dataset authorization, raw root publication, dynamic P4 terminal closure and final CAPTURED ordering remain separate gates.
...
`_p4_require_issued_authority`/`_p4_snapshot_integrity` with a closed
if/elif/else dispatch. GREEN discovered `authorize_capture_set` cannot reach a
genuine CAPTURED admission for the dynamic authority and disclosed this in
evidence 0182's `convergence_checkpoint`, naming two blockers: `_p4_checked_
dispatch`'s call to the module-level `_p4_reference_bytes` (whitelist has no
`root-capture-admission` case) and `_FixedCapturedAuthorizationContract.
_prepare` (reads `resolver._records` directly and requires
`pea`/`ces`/`bvp`-shaped fields the dynamic chain does not have).

Reading `dskit/pipeline/trust.py` directly for this ADR surfaces a fuller,
more precise picture than that two-function narrative:

1. `_p4_reference_bytes(value)` has **three** call sites, not one:
   `_p4_checked_dispatch` (trust.py:2520), `_LifecycleAuthorizationLedger.
   resolve_p4` (trust.py:5305), and `_LifecycleAuthorizationLedger.
   commit_p4_batch` (trust.py:5324). All three hard-check
   `value["kind"] in _P4_ADMISSION_SCHEMAS` (`{"action-execution-admission",
   "final-replay-admission"}` only); all three refuse a `root-capture-
   admission` ref before any resolver-specific code runs. `_p4_checked_
   dispatch`'s call is reached first, before `commit_p4_batch` is ever
   entered, so it is the actual first point of refusal today.
2. `_FixedCapturedAuthorizationContract._prepare` calls `_P4_CHECKED_RESOLVE
   = _FixedWormTrustedArtifactResolver._checked_resolve` and `_P4_RESOLVER_
   LOOKUP = _FixedWormTrustedArtifactResolver._lookup` as **hard module-level
   pins to the fixed class's own unbound methods** (trust.py:4203, 2349) --
   not through the resolver's own polymorphic `resolve()` (which already
   dispatches correctly for the dynamic resolver, per ADR-0143). Calling
   these pinned unbound methods with a `_DynamicP4TrustedArtifactResolver`
   instance as `self` reaches `_lookup`'s `for reference, raw in self.
   _records:` and fails with `AttributeError` -- the dynamic resolver has no
   `_records` slot (ADR-0143 Decision point 1/2, Phase 0 pin). This confirms
   the "reads `resolver._records`" finding but locates it precisely: it is
   not `_prepare` reading the attribute itself, it is `_prepare` calling two
   *hardcoded-fixed-class* helper pins that assume it.
3. `commit_p4_batch` **also** reads `resolver._records` directly, a third,
   previously undisclosed site: `cas = next(_hs_parse_canonical(raw) for
   ref, raw in resolver._records if ...)` (trust.py:5346-5347), used only to
   pull `cas["consumer_document_sha256"]`/`cas["purpose"]` into the final
   `decision` dict. This line is inline in `commit_p4_batch` itself, not
   reachable through any resolver method, so it cannot be fixed by resolver
   polymorphism alone.
4. `commit_p4_batch` contains a **fourth, latent, currently-unreachable-for-
   dynamic bug** in the already-merged ADR-0143 candidate: its final pre-
   commit re-check calls `_P4_CLOSE_ADMISSION(resolver, resolver._snapshot,
   admission_ref, (authority, captures, checked_runtime))` directly
   (trust.py:5382-5383) -- `_P4_CLOSE_ADMISSION = _p4_close_admission`, the
   **legacy, fixed-only** closure walk, not the polymorphic
   `resolver.close_admission(...)` the same method already uses correctly at
   two earlier points (trust.py:5334, 5352). No test can reach this line
   today for the dynamic authority (blocker 1 above refuses first), so
   ADR-0143's GREEN evidence never exercised it; a fix to blockers 1-3 alone
   would surface this as a new, undiagnosed failure at `authorize_capture_
   set`'s last step.
5. `resolve_p4` additionally hard-requires `authority_ref.get("kind") in
   ("action", "replay")` with `digest_field` fixed to exactly
   `"action_execution_admission_sha256"` for `kind="action"`
   (trust.py:5298-5300). Any `execution_authority_ref` shape this ADR invents
   for the dynamic chain must satisfy that exact literal check, or
   `resolve_p4` -- never named as extensible by ADR-0143 and not reasonably
   extensible without touching a `kind ∈ {"action","replay"}` closed set --
   refuses the dynamic authority's own idempotent re-resolution path.

None of these five points can be closed by touching the five forbidden
legacy-only symbols, and none requires touching them: `_FixedWormTrustedArtifactResolver`,
`_FixedTerminalArtifactVerifier`, `_FixedP4VerificationClock`,
`_FixedP4VerificationRevocations`, and `_p4_close_admission` are all left
completely unedited by this ADR's Decision below.

**Decision.**

1. `_p4_reference_bytes` gains a required second parameter and a closed,
   exact-`type(resolver)` if/elif/else, using the identical pattern ADR-0143
   already used for `_p4_require_issued_authority`/`_p4_snapshot_integrity`:
   `def _p4_reference_bytes(value, resolver):` -- `if type(resolver) is
   _FixedWormTrustedArtifactResolver: <today's exact
   _P4_ADMISSION_SCHEMAS-based body, verbatim, unedited> elif type(resolver)
   is _DynamicP4TrustedArtifactResolver: <new dynamic body, below> else:
   raise TypeError("exact broker-issued P4 capability is required")` --
   reusing the exact broker-identity exception text `_p4_require_issued_
   authority`'s own else arm already raises, since an unrecognized resolver
   at this call site is the same authority-identity failure. The dynamic
   body calls the existing, already-merged, self-contained `_p4_dynamic_
   reference_bytes(value)` (ADR-0143, trust.py:9309) for shape/sha256
   validation, then requires `value["role"] == "study-lifecycle" and
   value["kind"] in _P4_DYNAMIC_ADMISSION_SCHEMAS and value["schema"] ==
   _P4_DYNAMIC_ADMISSION_SCHEMAS[value["kind"]]`, raising `ValueError("P4
   admission kind, role and schema must agree")` -- the identical exception
   type and text the legacy branch already raises for the same failure --
   and returns `_hs_canonical_bytes(value)`, matching the legacy branch's
   return shape. A new closed mapping, `_P4_DYNAMIC_ADMISSION_SCHEMAS =
   MappingProxyType({"root-capture-admission":
   "dskit.root-capture-admission/v1"})`, is the dynamic sibling of
   `_P4_ADMISSION_SCHEMAS`; neither mapping is consulted by the other
   branch. All three call sites are updated to pass their already-in-scope
   resolver: `_p4_checked_dispatch` passes `authority._p4_resolver`;
   `resolve_p4` and `commit_p4_batch` pass their local `resolver`
   (`self._authority._p4_resolver`). `_p4_checked_dispatch`'s own dispatch
   logic is otherwise unchanged -- this is a one-argument call-site edit,
   not a new branch in `_p4_checked_dispatch` itself.
2. `_dynamic_p4_close_admission`'s existing inline `admission_ref["kind"] ==
   "root-capture-admission" and admission_ref["role"] == "study-lifecycle"
   and admission_ref["schema"] == "dskit.root-capture-admission/v1"` check
   (trust.py:9401-9406) becomes redundant defense-in-depth once point 1
   lands (by the time `close_admission` runs, `_p4_checked_dispatch` has
   already validated this). It is left in place unedited -- removing
   working, already-tested validation is out of scope for this ADR -- but
   Phase 0 may note the duplication.
3. `_LifecycleAuthorizationLedger.resolve_p4` requires no logic change beyond
   point 1's call-site edit, because Decision point 5 below pins the dynamic
   chain's `execution_authority_ref` to the byte-identical existing
   `{"kind": "action", "action_execution_admission_sha256": <sha256>}` shape
...
4. `_LifecycleAuthorizationLedger.commit_p4_batch` is extended at exactly
   ...
   method, the same category of change ADR-0143 already made to
   `_p4_require_issued_authority`/`_p4_snapshot_integrity`, now extended (by
   this ADR, explicitly) to `_p4_reference_bytes`, `resolve_p4`'s call site,
   and `commit_p4_batch`.
5. A new sibling class, `_DynamicCapturedAuthorizationContract(_Opaque)`, is
   added beside `_FixedCapturedAuthorizationContract`, mirroring its
   construction shape exactly (stateless, no token, no `__init__` override,
   one module-level singleton `_P4_DYNAMIC_CONTRACT =
   _DynamicCapturedAuthorizationContract()`). It is a new class, not a
   modification of `_FixedCapturedAuthorizationContract` -- Decision point 6
   of ADR-0143 ("commit_p4_batch's `_FixedP4Signer`/`_P4_VERIFICATION`
   signing infrastructure... remain shared, unedited") is honored because
   this class calls the **same** `_FixedP4Signer`/`_P4_SIGN`/`_P4_SIGNER`/
   ...
   - Module pins: `_P4_DYNAMIC_PREPARE =
     ...
     BATCH = _DynamicCapturedAuthorizationContract._verify`,
...
7. `_DynamicCapturedAuthorizationContract._prepare`'s `common`/`identity`
   substitute for the fixed contract's `pea`/`ces`/`bvp`-sourced fields with
   three digests already owned by this ADR's own closed graph -- not new
   artifacts, not a reintroduction of the pea/ces/bvp apparatus (ADR-0143's
   non-goal, preserved): `plan_evaluation_authorization_sha256 :=
   root_pis_ref["sha256"]`, `capture_expectation_set_sha256 :=
   dataset_g1_ref["sha256"]`, `broker_verified_plan_sha256 :=
   dataset_g2_ref["sha256"]`, `capture_admission_set_sha256 :=
   cas["capture_admission_set_sha256"]` (the last one identical in spirit to
   the fixed contract's own `admission["capture_admission_set_sha256"]`
   read, just sourced from the reconstructed `cas` instead of a resolved
   fixture). `identity["execution_authority_ref"]` is fixed to
   `{"kind": "action", "action_execution_admission_sha256":
   admission_ref["sha256"]}` -- the literal existing legacy shape for the
...
8. `_p4_dynamic_capture_admission_set` (ADR-0143, trust.py:9367, already
   ...
   own captures. `_validate_capture_request` already guarantees exactly one
   `consumer_document_sha256` across all captures in a batch (`len(documents)
   != 1` refuses); this ADR adds an equivalent `purpose` uniqueness
   requirement in the same reconstruction helper (point 6), refusing if the
   batch's captures do not share one `purpose` value, before populating
   `cas["purpose"]`. This is purely additive: `_dynamic_p4_close_admission`'s
   existing checks read neither top-level field and are unaffected by their
   presence. It is what lets `commit_p4_batch`'s shared decision-assembly
   line -- `decision.update(consumer_document_sha256=cas[
...
9. No new reserve kind, no new signer, no new key material, no change to
   ...
   (ADR-0143 Decision point 6, Open question 1) continue to apply to both
   authority instances unchanged. This ADR resolves that open question:
   the temporal admissibility gate for the dynamic authority is, and
   remains, the resolver's own fresh graph re-proof
   (`_P4_SNAPSHOT_INTEGRITY`/`resolver.close_admission`) inside
   `commit_p4_batch`'s locked critical section, not these shared fixture
   stamps, exactly as ADR-0143 already reasoned; no live-clock-stamped
   receipt is needed.
10. The returned `(record, session)` pair, the committed `self._root` entry,
    ...
    legacy-run-reuse refusals inherited from
    `_validate_capture_request`/`commit_p4_batch` unchanged. A successful
    dynamic `authorize_capture_set` therefore returns and records a
    CAPTURED admission of the identical shape the legacy authority already
    produces today, satisfying the non-negotiable requirement that this ADR
    not invent a different result shape.
11. `resolve_p4`, `_p4_entries`, `_p4_stream_documents`, `_legacy_captures`,
    `_request`, `_audit`, `read_p4_member`, `p4_receipt_digest` are
    confirmed, by direct reading, to require no further change: each
    operates on already-committed opaque `entry` tuples, canonical bytes, or
    generic descriptor/session state with no resolver-type or admission-kind
    assumption beyond the ones already closed by points 1 and 3.

**Closed ADR-0144 construction profile (for Phase 0 to pin or correct).**

- `_P4_DYNAMIC_ADMISSION_SCHEMAS = MappingProxyType({"root-capture-admission":
  "dskit.root-capture-admission/v1"})`, dynamic sibling of
  `_P4_ADMISSION_SCHEMAS`; consulted only inside `_p4_reference_bytes`'s new
  `elif` arm.
- `_DynamicCapturedAuthorizationContract(_Opaque)`: no `__slots__` beyond
...
- `common`/`identity` field substitution table (Decision point 7), restated
  ...
  |---|---|
  | `plan_evaluation_authorization_sha256` | `root_pis_ref["sha256"]` |
  | `capture_expectation_set_sha256` | `dataset_g1_ref["sha256"]` |
  | `broker_verified_plan_sha256` | `dataset_g2_ref["sha256"]` |
  | `capture_admission_set_sha256` | `cas["capture_admission_set_sha256"]` |
  | `execution_authority_ref` | `{"kind": "action", "action_execution_admission_sha256": admission_ref["sha256"]}` |
  | `subject_ref` | `{"kind": "action"}` (already fixed by ADR-0143 Decision point 7) |
- `_p4_dynamic_capture_admission_set`'s two new top-level fields:
  `consumer_document_sha256` sourced from `captures[0][2][
  "consumer_document_sha256"]` (already asserted unique across the batch by
  `_validate_capture_request`); `purpose` sourced from `captures[0][2][
  "purpose"]` after a new equivalent uniqueness assertion across
  `captures[*][2]["purpose"]`, raising the same `ValueError` family
  `_validate_capture_request`'s existing document-uniqueness check raises,
  with its own distinct message text (exact wording a Phase 0 deliverable).
- No change to `_SYNTHETIC_RESERVE_SCHEMA`, no new `reserve_uses` kind, no
  new `_P4_LOCAL_PUBLIC_KEYS` entry, no new `_FixedP4Signer` instance, no
  change to `_P4_BATCH_USES`'s four existing schema/self-field/usage
  triples (`captured-port`, `captured-receipt`, `captured-set`,
  `replay-capture-evidence` -- the last unused by the dynamic contract, not
  removed).

**Process.** Design proposal only. Requires independent preapproval review
and owner ADR approval, then Phase 0 (this crosses the same trust boundary
ADR-0143's Phase 0 already scoped for the dynamic authority, now reaching an
actually-effecting `authorize_capture_set` -> CAPTURED admission for the
first time), then focused RED/GREEN, two independent final lenses, affected
tests, and evidence, per docs/skills/implementation-workflow.md and
docs/skills/skeptic-review.md. No RED/GREEN, no merge, no code or test file
has been touched by this proposal.

**Non-goals.** Full EventEnvelope.v2 causality/ordering/provenance/
correction semantics; composed-tape verification; Packet 8 durable
consume-once; deployment; any edit to the fixed P4 corpus, its 500-ms clock,
`_FixedWormTrustedArtifactResolver`, `_FixedTerminalArtifactVerifier`, or
`_p4_close_admission`'s v1-only grammar, all of which remain byte-identical
and must continue to refuse dynamic v2/graph roots unchanged; reuse or
extension of the fixed `scope-intent`/`gate-evidence`/`scope-authorization`/
`stage-admission` historical-study apparatus for the dynamic domain (ADR-0143's
non-goal, preserved -- this ADR reuses three existing *field names* required
by the shared signed schema, never the pea/ces/bvp *artifacts* or their
resolution mechanism); any new reserve kind, signer, or key material; a
second `_LifecycleAuthorizationLedger` class or a subclass of
`_SyntheticP4CapturedAuthorizationAuthority`; cross-instance run-identity
...
- **Adopted (Decision below):** ADR-0129's accessor, applied to a single
  ...
  below. This is the smallest correct fix: it needs no `trust.py` edit and
  no `trust.py` import, composes `production/bundles.py`'s existing codec/
  verification seam behind one new function (Decision point 7) rather than
  a second one, reuses only already-merged, already-tested public entry
  points, and does not touch any of the five ADR-0143 forbidden-legacy
  symbols or ADR-0144's dynamic-dispatch code.
...
**Scope decision: one-hop vs three-hop composition.** The original
ADR-0127 Decision.2 design was a three-hop composition: a data capture, a
separate manifest-producer consumer of that data, a manifest publish of the
resulting outer-manifest root, and a third `ReplayRun` consumer of that
published manifest. That design was abandoned after three failed Phase 0
cycles (evidence 0174/0175/0176) because it requires production republishing
a second manifest root outside the fixed P4 corpus and outside the dynamic
authority's closed graph (ADR-0142 Decision point 3) -- an unbuilt
capability, not a bounded extension of anything already merged. This ADR
proves the achievable one-hop version instead: one dynamic-authority
CAPTURED admission supplies both the data and, via in-process construction
inside `compose_replay_tape` (Decision point 7), the manifest -- no separate
publish, no separate consumer, no second root. This is P7's actual, honest
closure criterion for a bounded, nondeployment, synthetic slice: it proves
`verify_causal_order` reachable end-to-end from resolved bytes, not that the
full three-hop production composition pipeline exists. A real multi-hop
composition -- a genuinely separate, separately-published and -captured
outer-manifest root, produced by a distinct consumer -- remains explicitly
out of scope here. It is ordered under the real F3 lane's own future work
(the forecast-capital-owned lane named in Context above), not this ADR. This
is recorded here as an explicit scope-narrowing decision, in this ADR
lineage's own established pattern (evidence 0179's
`owner_delegated_scope_decision` entries), rather than left implicit in the
"Correction" paragraph above.

**What is already merged and reused as-is, with no edit required by this
ADR** (contrast Decision point 7 below, which adds one new function and two
new small private helpers -- the only code this ADR itself contributes):
...
- `CapturedReplayTape._build` / `.canonical_bytes()` / `.parse()`,
  `verify_causal_order`, and `_check_event_envelope` (ADR-0145,
  `production/bundles.py`) -- `_check_event_envelope` is module-private but
  not test-private, and Decision point 7 below reuses it directly rather
  than the test-only `_envelope`/`_bytes` fixture builder in
  `tests/production/test_event_envelope_causal_order.py`, which remains that
  file's own, deliberately-isolated, unit-level fixture and is not reused
  here (this closes the preapproval review's Minor duplication-risk finding:
  envelope construction now has exactly one owner, Decision point 7's
  `compose_replay_tape`).
...
3. **Fixture root member layout.** The captured F4 root carries: one
   ...
   separate, larger, non-goal-fenced scope. **Implementation pin (Nit,
   closed here):** wherever the `dskit.composed-tape-roster-fixture/v1`
   literal is defined in `production/bundles.py` (Decision point 7 below),
   it carries a one-line code comment marking it test-only/placeholder --
   not a real `SourceRosterCapture.v1` and not a claim toward ADR-0130's
   undelivered broker.
4. **Resolve, don't assume, every `CapturedReplayTape` field.**
   `source_rank_policy_sha256` is the sha256 of the resolved roster
   member's own canonical bytes (recomputed by `compose_replay_tape` from
   bytes read via `read_member_bytes`, never a caller literal); each
   envelope's `source_rank` is that source's index in the resolved,
   rank-sorted roster. `data_capture_root` and `data_captured_receipt` are
   populated from the real capture -- the captured root's own identity
   digest (from the live `published` token `compose_replay_tape` is handed,
   computed inside the function, never accepted as one of its arguments)
   and the real `lifecycle_captured_receipt_sha256` for this exact
   `(stream, consumer_document_sha256)` -- never the placeholder
   ...
   invariant, enforced by `compose_replay_tape`'s own signature (Decision
   point 7), is that it is computed from the live capture, never supplied
   by a caller.
5. **Project, don't forward, raw-event bytes into envelopes.** Each resolved
   ...
   1), assembled and validated by `compose_replay_tape` itself (Decision
   point 7), via `_check_event_envelope` -- the same validated shape check
   `verify_causal_order` applies per-position -- not by reusing
   `test_event_envelope_causal_order.py`'s test-private `_envelope`/`_bytes`
   fixture builder, which remains that file's own, deliberately-isolated
   construction path and is not called from production code. `source_rank`
   and `source_rank_policy_sha256` come from step 4, resolved, not fixture
   literals; `exchange_ms`/`receive_ms`/`source_provenance_tag`/
   `source_timezone_tag`/`correction_position`/`corrects_event_id`/
   `prior_envelope_sha256` remain synthetic, fixture-declared metadata the
   caller supplies to `compose_replay_tape` per envelope, exactly as
   ADR-0145 already scopes them (Decision point 1: opaque, never
   interpreted; real F1/F2 derivation of these fields is that ADR's own
   non-goal, unchanged). The minimal positive fixture is one straight-line
   tape -- `correction_position=0`, `corrects_event_id=None`,
   ...
   member's byte after capture and before `compose_replay_tape`'s own
   re-derivation, and confirm `verify_causal_order`'s digest check -- or, if
   tampering is caught earlier, `read_member_bytes`'s own digest refusal --
   still fires; the exact tamper point is a Phase 0 deliverable).
6. **Build, round-trip, then verify -- using only already-merged functions.**
   `compose_replay_tape` (Decision point 7) calls
   `CapturedReplayTape._build(data_capture_root, data_captured_receipt,
   source_rank_policy_sha256, ordered_envelope_digests)`, then
   `.canonical_bytes()` followed by `CapturedReplayTape.parse(...)` (a
   genuine default-deny round trip, not a construction shortcut), then
   `verify_causal_order(tape, ordered_envelope_bytes)` with the same
   resolved envelope bytes the digests were computed from, and returns
   `verify_causal_order`'s own verified tape on success. `_build`,
   `canonical_bytes`, `parse`, and `verify_causal_order` themselves are
   unchanged and already merged and public (or, for `_build`, already
   reached directly by `test_captured_replay_tape.py`, an established
   whitebox pattern this proof continues); Decision point 7 below is the
   one place that composes them into a new callable.
7. **One new pure production function, `compose_replay_tape`, in
   `production/bundles.py`.** This closes the preapproval review's Critical
   finding: RE-ENTRY's stated gate is "constructing a runtime composed-tape
   capability that calls `verify_causal_order` end-to-end", language naming
   a callable capability, not merely a passing test. `compose_replay_tape`
   is that capability -- the actual "composed-tape capability" P7's gate
   ...
   **Signature:**
   ...
   ```
   `record` (`CapturedAuthorizationRecord`), `session` (`LaunchSession`),
   and `published` are the exact three values one `authorize_capture_set`
   call already returned (`record`, `session`) plus the `published` token
   the caller captured (ADR-0129's own shapes, unchanged) --
   `compose_replay_tape` calls only their existing public
   methods/attributes and imports no symbol from `dskit.pipeline.trust`,
   so it adds no new cross-module dependency and does not disturb the
   pipeline/production layering boundary ADR-0145 Decision point 5 already
   relies on ("the only production module imported here is `base`",
   `bundles.py`'s own module docstring). `consumer_document_sha256` is the
   frozen consumer document's own digest (`frozen.document_sha256` /
   `port["consumer_document_sha256"]`, ADR-0129's existing shape).
   `roster_relative_path` is the `source_roster.json` member's relative path
   in the sealed manifest (Decision point 3). `raw_event_members` is an
   ordered sequence, one item per envelope, **in the exact intended final
   tape order** (position `i` in the sequence becomes tape position `i`; no
   sorting is performed by this function -- Decision point 5's order-key
   check is `verify_causal_order`'s job, downstream), each item a mapping:
   `relative_path` (str, the raw-event/v1 member's path), `exchange_ms`,
   `receive_ms` (nonnegative int), `source_provenance_tag`,
   `source_timezone_tag` (nonempty str), `correction_position` (nonnegative
   int), `corrects_event_id` (str or `None`), `prior_envelope_sha256` (sha256
   or `None`) -- exactly the seven ADR-0145 Decision point 1 fields Decision
   point 5 above already scopes as caller-supplied, synthetic, fixture-
   declared metadata.

   **Steps** (all inside the function; the caller supplies only the six
   arguments above):
   1. `stream = published.sealed.prepared.stream_id` -- the committed
      publication stream identity `lifecycle_captured_receipt_sha256`
      requires, recovered from the `published` token's own public
      attribute path, not a seventh argument.
   2. `roster_bytes = record.read_member_bytes(session, published,
      consumer_document_sha256, roster_relative_path)`.
   3. Parse `roster_bytes` as JSON and validate it with a new, small,
      private `_check_composed_tape_roster_fixture(value)` (added beside
      `_check_tape`/`_check_event_envelope`, same accumulate-then-raise
      style): `schema_version` must equal the literal
      `dskit.composed-tape-roster-fixture/v1`; `sources` must be a list of
      `{source_id: nonempty str, rank: nonnegative int}` objects, unique
      `source_id`, and `rank` values forming exactly `0..len(sources)-1` in
      list order (i.e., already rank-sorted, no gap, no duplicate -- Decision
      point 3's "sorted by rank from 0" is enforced here, not assumed). Any
      problem raises `ProductionError` naming every problem found.
   4. `source_rank_policy_sha256 = hashlib.sha256(roster_bytes).hexdigest()`
      (Decision point 4: the resolved roster member's own bytes' digest,
      never a caller literal).
   5. `source_rank_by_id = {item["source_id"]: item["rank"] for item in
      roster_value["sources"]}` (Decision point 4: each source's index in
      the resolved, rank-sorted roster).
   6. For each `entry` in `raw_event_members`, in order:
      a. `raw_bytes = record.read_member_bytes(session, published,
         consumer_document_sha256, entry["relative_path"])`.
      b. Parse as JSON and validate with a new, small, private
         `_check_raw_event_member(value)`: the closed six-key
         `dskit.raw-event/v1` shape (`schema_version` literal, `source_id`/
         `event_id` nonempty str, `source_sequence`/`availability_ms` int,
         `payload_sha256` a sha256 digest) -- mirroring ADR-0132's
         `_SYNTHETIC_RAW_EVENT_KEYS` shape (`trust.py:7072`), reimplemented
         locally, not imported, solely because `production/bundles.py` must
         not import `dskit.pipeline.trust` (the same layering boundary step
         1 cites); this is a bounded, ADR-scoped restatement of an existing
         six-field shape, not new grammar, and is distinct from the Minor
         finding's envelope-construction duplication (that was two *envelope*
         builders; this is one small *raw-event* shape check that cannot be
         imported across the layering boundary).
      c. `source_rank = source_rank_by_id[value["source_id"]]`; a
         `source_id` absent from the roster accumulates into the raised
         `ProductionError` rather than raising a bare `KeyError`.
      d. Assemble the 15-field `dskit.event-envelope/v2` object (ADR-0145
         Decision point 1): `schema_version`, `source_id`, `event_id`,
         `source_sequence`, `payload_sha256`, `availability_ms` carried
         verbatim from the parsed raw-event member; `source_rank` (6c);
         `source_rank_policy_sha256` (step 4); `exchange_ms`, `receive_ms`,
         `source_provenance_tag`, `source_timezone_tag`,
         `correction_position`, `corrects_event_id`, `prior_envelope_sha256`
         taken verbatim from `entry`.
      e. Validate the assembled object with `_check_event_envelope`
         (already in `production/bundles.py`, not test-private) -- the
         *same* validated path `verify_causal_order` itself applies
         per-position later (Decision point 5 above). Any problem raises,
         position included in the message.
      f. `envelope_bytes = _canonical_bytes(envelope)`; append to
         `ordered_envelope_bytes`; append
         `hashlib.sha256(envelope_bytes).hexdigest()` to
         `ordered_envelope_digests`.
   7. Compute `data_capture_root` from the live `published` token, inside
      this function -- never accepted as an argument, closing the risk of a
      caller injecting an arbitrary root digest. The exact bytes hashed (the
      F4 publication receipt's own self digest vs.
      `_canonical_hash(published.descriptor)`) remains the Phase 0 pin
      Decision point 4 already named; `compose_replay_tape`'s contract is
      only that the value is computed here, from the live capture.
   8. `data_captured_receipt = record.lifecycle_captured_receipt_sha256(
      stream, consumer_document_sha256)` (`stream` from step 1).
   9. `tape = CapturedReplayTape._build(data_capture_root,
      data_captured_receipt, source_rank_policy_sha256,
      ordered_envelope_digests)`, then `reparsed =
      CapturedReplayTape.parse(tape.canonical_bytes())` -- Decision point
      6's genuine round trip, unchanged.
   10. `verify_causal_order(reparsed, ordered_envelope_bytes)`. On success
       (the function's own contract: returns `None`), `compose_replay_tape`
       returns `reparsed`, the verified tape. On any violation, the
       `ProductionError` `verify_causal_order` raises propagates unchanged --
       `compose_replay_tape` adds no second error-handling layer around it.

   **Non-duplication guarantee.** No file other than `production/bundles.py`
   constructs a `dskit.event-envelope/v2` object or a `CapturedReplayTape`
   value under this ADR; the Decision point 9 test calls
   `compose_replay_tape` and inspects its return value and raised errors, it
   does not reimplement steps 1-10. `compose_replay_tape` is exported
   alongside `verify_causal_order` (added to `__all__`), so it is equally
   reachable from a test fixture or a later real composed-tape caller
   (`ReplayRun`'s eventual wiring, Non-goals below), matching
   `verify_causal_order`'s own "callable equally from a test fixture or a
   later composed-tape caller" framing (ADR-0145 Decision point 3).
8. **Dynamic-authority RED requirement.** `read_member_bytes` and
   `lifecycle_captured_receipt_sha256` are claimed resolver-type-agnostic
   "by inspection" (Context above, re-reading `trust.py` directly); that
   claim is not itself a test. Phase 0's RED matrix **must** include at
   least one case that drives the full chain -- `authorize_capture_set` ->
   `record`/`session` -> `compose_replay_tape` -> `verify_causal_order` --
   end-to-end through the **dynamic** authority (ADR-0143/0144's
   `trust._development_dynamic_p4_broker`/
   `trust._DynamicP4TrustedArtifactResolver`, the same construction
   `_adr143_issued_case` already uses), not only the fixed/legacy authority.
   This is this ADR's own positive proof case -- Decision point 2 names the
   dynamic authority specifically -- and it must not be allowed to quietly
   default to covering only the easier fixed-authority path the way a
   RED matrix can silently do when both are nominally "a P4 capture." A
   fixed-authority case may additionally exist (e.g., as a second
   comparison case or a regression guard) but does not substitute for the
   dynamic-authority case this point requires.
9. **Test-only fixture fabrication, file placement.** What remains
   test-only is the fixture *fabrication* -- choosing the roster's
   `source_id`s/ranks, the raw-event members' field values, and the per-
   envelope `exchange_ms`/`receive_ms`/`source_provenance_tag`/
   `source_timezone_tag`/`correction_position`/`corrects_event_id`/
   `prior_envelope_sha256` metadata Decision point 7's `raw_event_members`
   argument carries -- and driving one real F4 produce/seal/publish/
   `authorize_capture_set` lifecycle to obtain `record`/`session`/
   `published`. `deployment_eligible=false` throughout; the fixture bytes,
   layout, and driving code are exactly as bounded and synthetic as every
   prior P7 slice's. The test then calls `compose_replay_tape` (Decision
   point 7) rather than repeating steps 1-10 inline (this is what closes
   the Minor finding, alongside Decision point 7 itself). Two placements
   remain available and the choice is a Phase 0 pin: (a) append to
   ...
   `dskit.production.bundles` locally for the `compose_replay_tape`/
   `verify_causal_order` calls (test code is not bound by
   `production/test_purity.py`'s "function depth only" rule or `pipeline`'s
   "never import production" rule, both of which bind package code, not
   tests); or (b) a new `tests/production/test_composed_replay_tape.py`
   that imports the needed `trust.py` fixture helpers directly. This ADR
   recommends (a), since it avoids re-deriving working fixture construction
   across two files, but defers the final choice to Phase 0.
10. **No `trust.py` edit, and no `trust.py` import.** Every `trust.py`
    symbol this Decision touches (`CapturedAuthorizationRecord`,
    `LaunchSession`, `authorize_capture_set`, the ADR-0143/0144 dynamic
    authority) already exists, is already merged, and is already exercised
    by a passing test; `compose_replay_tape` reaches them only through
    values the caller already holds and their own public
    methods/attributes (Decision point 7's signature), never through an
    import of `dskit.pipeline.trust` from `production/bundles.py`. This
    ADR's total code delta is therefore exactly one new production function
    plus two new small, pure, private shape-check helpers
    (`_check_composed_tape_roster_fixture`, `_check_raw_event_member`), all
    in `production/bundles.py`, plus the Decision point 9 test -- lower-risk
    than any prior P7 slice that edited `trust.py` directly, and strictly
    smaller than the three-hop design the Scope decision subsection above
    retires.

**Process.** Design proposal only. Requires independent preapproval review
and owner ADR approval, then Phase 0 skeptic review (to pin the exact
Decision point 4 digest-tamper location, the Decision point 7 `data_capture_root`
derivation, and the Decision point 9 file placement), then focused RED/GREEN
with the synthetic fixture described above -- including the Decision point 8
dynamic-authority RED case -- two independent final lenses
(correctness/authority, then tests/integration), affected tests, and
...
**a real multi-hop, three-consumer composition chain** matching the original
ADR-0127 Decision.2 design -- a genuinely separate, separately-published and
-captured outer-manifest root produced by a distinct manifest-producer
consumer -- retired by the Scope decision subsection above; this is the
precise replacement for any earlier "no production code" framing, now that
Decision point 7 adds `compose_replay_tape`: what stays out of scope is the
*multi-hop composition*, not production code as such. Also out of scope: any
extension to the dynamic authority's closed 12-reference graph; exercising
ADR-0128 multi-consumer capture as a requirement (available, not required,
per Decision point 2); resurrecting ADR-0130's undelivered
`SourceRosterCapture.v1`/`RawEventDatasetCapture.v1`/
`DatasetCaptureAuthorization.v1` broker; the full master F3 `EventEnvelope.v2`
lane (ADR-0145's own non-goals, unchanged); real replay, backtest, paper or
live operation; Packet 8 durable consume-once (F5A-R23); deployment. Any
production surface beyond the single `compose_replay_tape` function and its
two small private shape-check helpers (`_check_composed_tape_roster_fixture`,
`_check_raw_event_member`) -- no new public export beyond `compose_replay_tape`
itself, no new class, and no edit to `CapturedReplayTape`, `_check_tape`,
`_check_event_envelope`, or `verify_causal_order` themselves (Decision
point 6's "using only already-merged functions" is unchanged; they are
composed, not modified). Any edit to `trust.py`'s dynamic P4 authority, the
five ADR-0143 forbidden-legacy symbols, or ADR-0144's dynamic-dispatch code
-- none of that is touched, referenced beyond read-only reuse, or depended
...
* `MeanIntervalEstimator(ABC)` — the doorway. `interval(evidence, confidence=)`
  is a TEMPLATE method a member never overrides: it owns the screens, the
  minimum-units refusal, the result invariants and determinism. Two
  `@abstractmethod` hooks, one job each — `mean_and_se(evidence)` and
  `bounds(evidence, mean, se, confidence)` — plus a `minimum_units` hook,
  because "fewer units than the method can support" is the METHOD's fact.
* `MeanIntervalResult` — frozen; cannot exist with `low > mean` or
  ...
  consumer's sign convention, which dskit does not have.
...
* `ESTIMATORS` / `register_estimator` / `estimator` — the registry idiom
  `stats.register_correction` set and `false_signal.py` reused. The registry
  holds CLASSES, because the family is an object with hooks.
* `ClusterBootstrapInterval` additionally refuses when a stated `overlap_steps`
  would SPAN a unit boundary (overlap >= the shortest unit's contiguous run).
  That is `attempts.py`'s session doctrine enforced rather than assumed: units
  whose labels reach into the next unit are not independent units.
...
p-value and wrong for a number a capital constraint consumes: a consumer that
...
`stats.student_t_sf`. The Student tail stays owned by `stats.py`; the new module
bisects on it for the quantile. Copying the continued fraction into a second
module is the defect CLAUDE.md's "a function is never repeated across modules"
names, and `false_signal.clopper_pearson_upper` bisecting on a public `stats`
tail is the precedent being followed.
