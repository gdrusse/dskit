...
## ADR-0063 — Skill is a pooled Diebold–Mariano gap, not a rejection count
...
- ADR-0063's pooled Diebold–Mariano t needs `d_t`; `RESULT-P5.md` had to
...
ADR-0063 began this by writing its gaps to `skill.json`. That artifact
...
5. `runs.score_walk` reads these rows and rebuilds `d_t` and `q_f` from
   them, so ADR-0063's verdict is computed from the evidence rather than
...
Clark–West test. This REPLACES ADR-0063's in-memory JSON payload, so the
...
unaffected** — they saved no rows and stay half-scored under ADR-0063
...
gap is the whole question, and ADR-0063 scores only the size half of it.
...
- **Size — `calibration_slope`.** Mincer–Zarnowitz: fit y = a + bŷ per
  ...
  rule ADR-0063 uses, so both bands are built the same way. Report the
...
retaining at least half the raw one. PASS(magnitude) stays ADR-0063,
untouched.
...
a lucky one score the same. ADR-0063 says how one cell is scored; it
...
their own null, which ADR-0063's studentised statistic is.

**Decision.** `dskit/pipeline/attempts.py`, in three pieces, sitting ON
TOP of ADR-0063 and never replacing it.
...
A cell passes only when ALL of: ADR-0063 passed **unchanged**; the
