...
with its own group universe on the P10 geometry, so the document is
`p12-64-asset-modelability` over 64 fit symbols in five groups declared
largest-first (`d`, `e`, `a`, `c`, `b`); the identity moves, as a cohort
...
dividends — and the resulting exclusions, if any, are recorded in the memo
and the journal; a name is excluded only when its tape carries a real-world
discontinuity the split-only adjustment leaves in place inside the walk's
window.
