...
architecture PASS 0C/0M (12 nits). Nits/minors not edited. ADR-0119
(renumbered from this branch's own ADR-0117 during merge — that number
was independently taken by Gate 6, merged first) stays **proposed**.
Identity `5adac27e…`. Focused: 94 passed (`test_replay` +
`test_nodes_capital`); ruff clean. Five documented Gate 2 `test_configs`
failures remain.

Memos:
`children/intraday_equities/docs/memos/2026-09-10-final-model-replay-gate5-skeptic-method-cycle7.md`
and `-architecture-cycle7.md` (memos keep their original "ADR-0117"
citation as written at the time; only the live decision-log/config/code
citations were renumbered).

**Next (owner):** accept or amend ADR-0119; do not edit nits (restarts
the loop). Gates 3, 4, 7 and Phase 5 items 3–5 remain (Gate 6 already
merged and accepted as ADR-0117).
