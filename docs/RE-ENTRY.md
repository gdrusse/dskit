...
**Merged `claude/prod-lane-20260917` into `main`.** The slice had branched at
`e989dff`; before landing it caught up the 34 commits main gained since (P18
breadth cohort, ADR-0149 sklearn reduction, ADR-0151/0152 uncertainty layer).
Code and tests from the two lanes touch disjoint files, so the only
hand-resolved conflicts were the two append-only docs, both preserving every
side. The production-lane work itself: ADR-0150 (fold/composition seams),
...
duplicate `## ADR-` header, and the hand-resolved decision-log kept all nine
...
**13122 passed / 254 skipped / 1 xfailed / 0 failed**. The single xfail is
