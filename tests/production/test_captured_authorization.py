...
def test_both_facades_issue_the_same_authority_bound_p4_transaction(facade, replay, count):
    p4._closure_ready()
    ...
    verifier = verifier_module.HistoricalStudyVerifier(broker)
    doorway = verifier if facade == "verifier" else verifier_module.HistoricalStudyCaptureDriver(verifier)
    record, session = doorway.authorize_capture_set(captures, graph.selected, **runtime)
    assert type(record) is trust.CapturedAuthorizationRecord
    assert type(session) is trust.LaunchSession and session._kind == "captured-authorization-v2"
    assert doorway.authorize_capture_set(captures, graph.selected, **runtime) == (record, session)
...
def test_v1_p4_refusal_has_no_effect_and_does_not_burn_legacy_admission(facade):
    ...
    with pytest.raises(TypeError, match="broker-issued P4 authority capability is required"):
        method(
...
def test_p4_facade_rejects_copied_state_and_late_authority_substitution(facade):
    ...
    method = getattr(type(doorway), "authorize_capture_set", None)
    assert callable(method), "Matrix v7 checked P4 facade method is missing"
    with pytest.raises(ValueError, match="opaque bound P4 authority and verifier are required"):
